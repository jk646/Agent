...
if ($LASTEXITCODE -ne 0) {
    throw "Read Folder Tool workspace does not exist: $Workspace"
}

& wsl.exe -d $Distro -u root --cd $linuxRepo -- /usr/local/go/bin/go run ./cmd/read-folder-tool --socket $Socket --workspace $Workspace
exit $LASTEXITCODE
